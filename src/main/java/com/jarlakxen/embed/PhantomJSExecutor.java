...
public class PhantomJSExecutor {
    ...
    public String execute(File sourceFile, String... args) {
        try {
            String cmd = this.phantomJSExecutablePath + " " + sourceFile.getAbsolutePath() + " "
        ...
        try {
            ...
            if (hasPrefix && hasPostfix) {
                output = output.substring(PHANTOMJS_CONSOLE_PREFIX.length(),
                    output.length() - PHANTOMJS_CONSOLE_POSTFIX.length());
    ...
    private String getPhantomJSExecutablePath() {
        ...
        String targetPath = this.configuration.getTargetInstallationFolder() + "/phantomjs";
        ...
        try {
            ...
            if (phantomJsCompressedFile.getName().endsWith(".zip")) {
                ...
                archiveInputStream = new TarArchiveInputStream(new BZip2CompressorInputStream(new FileInputStream(
                    phantomJsCompressedFile)));

            } else if (phantomJsCompressedFile.getName().endsWith(".gz")) {

                archiveInputStream = new TarArchiveInputStream(new GzipCompressorInputStream(new FileInputStream(
                    phantomJsCompressedFile)));
            ...
            while ((entry = archiveInputStream.getNextEntry()) != null) {
                if (entry.getName().endsWith("/bin/phantomjs")) {
                    ...
                    new File(this.configuration.getTargetInstallationFolder()).mkdirs();

                    // Create empty binary file
                    File output = new File(this.configuration.getTargetInstallationFolder() + "/phantomjs");
                    if (!output.exists()) {
    ...
    private Boolean checkPhantomJSInstall(String path) {
        try {
            ...
            if (this.configuration.getVersion().equals(PhantomJSVersion.fromValue(processOutput.substring(0, 5)))) {
                return true;
            }
        } catch (Exception e) {
            LOGGER.warn(e);
